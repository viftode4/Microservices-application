...
public class JwtRequestFilterTests {
    ...
    private static final String TOKEN = "randomtoken123";
    ...
    public void correctToken() throws ServletException, IOException {
        ...
        String token = TOKEN;
        String user = USER;
        when(mockRequest.getHeader(AUTH)).thenReturn("Bearer " + token);
        ...
        when(mockJwtTokenVerifier.getRoleFromToken(any())).thenReturn(String.valueOf(UserRole.Employee));
    ...
    public void nullToken() throws ServletException, IOException {
        // Arrange
        when(mockRequest.getHeader(AUTH)).thenReturn(null);
    ...
    public void invalidPrefix() throws ServletException, IOException {
        ...
        String token = TOKEN;
        String user = USER;
        when(mockRequest.getHeader(AUTH)).thenReturn("Bearer1 " + token);
        when(mockJwtTokenVerifier.validateToken(token)).thenReturn(true);
        when(mockJwtTokenVerifier.getNetIdFromToken(token)).thenReturn(user);
    ...
    public void noPrefix() throws ServletException, IOException {
        ...
        String token = TOKEN;
        String user = USER;
        when(mockRequest.getHeader(AUTH)).thenReturn(token);
        when(mockJwtTokenVerifier.validateToken(token)).thenReturn(true);
        when(mockJwtTokenVerifier.getNetIdFromToken(token)).thenReturn(user);
