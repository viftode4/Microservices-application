...
@ActiveProfiles({"test", "mockPasswordEncoder", "mockTokenGenerator", "mockAuthenticationManager",
        "mockRoleManager", "mockTokenVerifier"})
...
public class UsersTests {
    ...
    @Test
    public void registerCandidate_asHr_good() throws Exception {

        when(mockRoleManager.getRole()).thenReturn(String.valueOf(Role.HR));
        ...
        when(mockPasswordEncoder.hash(newTestPassword)).thenReturn(existingTestPassword);
        ...
        // Act
        ResultActions resultActions = mockMvc.perform(post("/hr/registerCandidate")
                .contentType(MediaType.APPLICATION_JSON)
                .content(JsonUtil.serialize(model))
                .header("Authorization", "Bearer " + testUser.toString()));
    ...
    //test setHr method
    @Test
    public void setHr_good() throws Exception {
        ...
        AppUser appUser = new AppUser(testUser, existingTestPassword);
        userRepository.save(appUser);
        ...
        // Act
        ResultActions resultActions = mockMvc.perform(post("/hr/setHr")
                .contentType(MediaType.APPLICATION_JSON)
                .content(JsonUtil.serialize(model)));
        ...
        appUser = userRepository.findByNetId(testUser).get();
        assertThat(appUser.getRole()).isEqualTo(Role.HR);
    }
